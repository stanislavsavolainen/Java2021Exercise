...
public class FindSmalestNumberDistance {
	...
	final int absoluteMaxElement = 1000;
	...
	public int find( int numberArray[] , boolean allowNegativeNumbers ){
		...
		if(numberArray != null) {
			...
			// means that we don't continue code under this condition, if we reach allowed element limit in array
			if( numberArray.length > absoluteMaxElement ) return -3; 
			
			HashSet<Integer> removeDuplicat = new HashSet<Integer>();
			for ( int elementNumber : numberArray ) removeDuplicat.add( elementNumber);
			
			int [] handledNumberArray =  new int[ removeDuplicat.size() ];  
			
			int collectionLoopIndex = 0;
			for( Integer collectionElement : removeDuplicat  ) handledNumberArray[collectionLoopIndex++] = collectionElement;
					...
					if(index <  handledNumberArray.length - 1 ) {
						...
						if ( ! allowNegativeNumbers ) {
							...
							//else {
								...
								if( (  handledNumberArray[index] < 0 &&  handledNumberArray[index + 1] < 0 ) ){
						...
						//for( int indx = ( numberArray[index] + 1 ) ; indx < ( numberArray[index + 1] ) ; indx++) System.out.print( " " + indx + "," );
