...
public class SimulateBackendData {
	...
	public String generateProfileJSON( int userId ){
		...
		try{
			...
			profileJSONObjectstr += "{ "+
					...
					" , 'socialsecuritynumber': '"+userlist.get(userId).getSocialSecurityNumber()+
					"'}";
	...
	public void addNewUser( JSONObject newUser ){
		...
		try{
			...
			//String tmpFullName = newUser.get("fullname").toString();
			//String tmpAddress = newUser.get("address").toString();
			//String tmpSocialNumber = newUser.get("socialsecuritynumber").toString();
			//String tmpNationality = newUser.get("nationality").toString();
